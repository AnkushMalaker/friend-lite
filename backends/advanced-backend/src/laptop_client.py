        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(auth_url, data=form_data) as response:
                if response.status == 200:
                    ...
                    error_detail = await response.text()
                    raise Exception(f"Authentication failed with status {response.status}: {error_detail}")
...
def validate_auth_args(args):
    ...
    if args.username and not args.password:
        raise ValueError("Username provided but password missing: Both --username and --password are required")
    
    if args.password and not args.username:
        raise ValueError("Password provided but username missing: Both --username and --password are required")
...
async def main():
    ...
    if args.token:
        ...
        token = args.token
        print(f"Using provided JWT token: {token[:20]}...{token[-10:] if len(token) > 30 else token}")
    
    elif args.username and args.password:
        # Authenticate with username/password to get token
        print(f"Authenticating with username: {args.username}")
        try:
            token = await authenticate_with_credentials(args.host, args.port, args.username, args.password)
            print(f"Authentication successful! Received token: {token[:20]}...{token[-10:] if len(token) > 30 else token}")
