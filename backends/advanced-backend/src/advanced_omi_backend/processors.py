...
class ProcessorManager:
    ...
    def track_processing_task(self, client_id: str, stage: str, task_id: str, metadata: Dict[str, Any] = None):
    ...
    def track_processing_stage(self, client_id: str, stage: str, status: str, metadata: Dict[str, Any] = None):
    ...
    def get_all_processing_status(self) -> Dict[str, Any]:
        ...
        return {
            client_id: self.get_processing_status(client_id)
            ...
            while not self.shutdown_flag:
                try:
                    ...
                    item = await asyncio.wait_for(
                        self.audio_queue.get(), timeout=30.0
                    )
    ...
    async def _transcription_processor(self):
        ...
        try:
            while not self.shutdown_flag:
                try:
                    item = await asyncio.wait_for(
                        self.transcription_queue.get(), timeout=30.0
                    )
    ...
    async def _memory_processor(self):
        ...
        try:
            while not self.shutdown_flag:
                try:
                    item = await asyncio.wait_for(
                        self.memory_queue.get(), timeout=30.0
                    )
    ...
    async def _process_memory_item(self, item: MemoryProcessingItem):
        ...
        transaction_id = tracker.create_transaction(
            item.user_id, item.client_id, item.audio_uuid
        )
        tracker.track_event(
            transaction_id,
        ...
        try:
            ...
            if memory_result:
                audio_logger.info(f"✅ Successfully processed memory for {item.audio_uuid}")
                tracker.track_event(
                    transaction_id,
    ...
    async def _cropping_processor(self):
        ...
        try:
            while not self.shutdown_flag:
                try:
                    item = await asyncio.wait_for(
                        self.cropping_queue.get(), timeout=30.0
                    )
