...
class PersistentQueue:
    ...
    async def nack(
        self, message_id: str, error_message: str = "", delay_seconds: float = 0
    ) -> bool:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                if new_retry_count <= max_retries:
                    ...
                    self.queue_tracker.update_item_status(
                        message_id, QueueStatus.RETRY, error_message
                    )

                    logger.info(
                        f"Retrying message {message_id} in {backoff_delay}s (attempt {new_retry_count})"
                    )
                    ...
                    # Update queue tracker
                    self.queue_tracker.update_item_status(
                        message_id, QueueStatus.DEAD_LETTER, error_message
                    )

                    logger.warning(
                        f"Message {message_id} moved to dead letter queue after {new_retry_count} attempts"
                    )
    ...
    async def cleanup_completed_messages(self, hours_old: int = 24) -> int:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                logger.info(
                    f"Cleaned up {deleted_count} completed messages older than {hours_old} hours"
                )
    ...
    async def requeue_dead_letter_message(self, message_id: str, max_retries: int = 3) -> bool:
