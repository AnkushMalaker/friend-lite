...
class QueueTracker:
    ...
    def update_item_status(
        self, item_id: str, status: QueueStatus, error_message: Optional[str] = None
    ) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                processing_completed_at = (
                    now if status in [QueueStatus.COMPLETED, QueueStatus.DEAD_LETTER] else None
                )
    ...
    def get_item(self, item_id: str) -> Optional[QueueItem]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute("SELECT * FROM queue_items WHERE id = ?", (item_id,))
                row = cursor.fetchone()
                if row:
    ...
    def get_pending_items(
        self, queue_type: QueueType, client_id: Optional[str] = None, limit: int = 100
    ) -> List[QueueItem]:
    ...
    def get_stale_processing_items(
        self, queue_type: QueueType, timeout_seconds: int = 300
    ) -> List[QueueItem]:
