...
from .persistent_queue import MessagePriority, PersistentQueue, get_persistent_queue
from .queue_tracker import QueueStatus, QueueTracker, QueueType, get_queue_tracker
...
async def requeue_dead_letter_message(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404, detail=f"Dead letter message {message_id} not found"
            )
...
async def get_system_overview(
    ...
    try:
        ...
        total_queue_items = sum(sum(queue_data.values()) for queue_data in queue_stats.values())

        healthy_services = sum(
            1 for service in health_stats["services"].values() if service["status"] == "healthy"
        )

        open_circuits = sum(1 for circuit in circuit_stats.values() if circuit["state"] == "open")
