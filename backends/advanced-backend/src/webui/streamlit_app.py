            ...
            if st.session_state.get('show_admin_memories', False):
                ...
                if admin_memories_response:
                    ...
                    with col3:
                        ...
                        st.metric("Debug Tracker", "✅" if stats.get('debug_tracker_initialized') else "❌")
                    ...
                    if view_mode == "📋 By User":
                        ...
                        if user_memories:
                            ...
                            for user_id, user_memory_list in user_memories.items():
                                ...
                                with st.expander(f"👤 {user_email} ({user_id}) - {memory_count} memories", expanded=False):
                                    if user_memory_list:
                    ...
                    if enriched_memories:
                        ...
                        if analysis_data:
                            ...
                            for i, memory in enumerate(enriched_memories):
                                ...
                                with st.expander(f"🧠 {title_text} | {compression_ratio}% compression", expanded=False):
                                    col1, col2 = st.columns(2)
...
if tab_debug is not None:
    with tab_debug:
        ...
        if not st.session_state.get('authenticated', False):
            ...
            with col2:
                if st.button("🔍 Analyze Session", key="analyze_transcript_memory", disabled=not audio_uuid_input.strip()):
                    st.session_state['transcript_analysis_uuid'] = audio_uuid_input.strip()
                    st.session_state['transcript_analysis_loaded'] = True
            with col3:
                if st.button("📋 Show All Transcripts", key="btn_show_all_transcripts", help="Show all transcripts vs memories for comprehensive analysis"):
                    ...
                    if transcript_analysis:
                        ...
                        with st.expander("📋 Session Information", expanded=True):
                            ...
                            with col2:
                                ...
                                st.metric("Processing Status", "✅ Success" if success else "❌ Failed")
                            ...
                            **Statistics:**
                            ...
                            if extractions:
                                ...
                                if analysis.get('empty_results'):
                                    st.info("🤔 LLM determined no memorable content in this conversation")
                                else:
                        ...
                        with col3:
                            ...
                            st.metric("Processing OK", "✅ Yes" if processing_successful else "❌ No")
                        ...
                        if has_transcript and processing_successful:
                            if has_memories:
                                ...
                                if compression_ratio > 50:
                                    st.warning("⚠️ High compression ratio - may indicate poor memory extraction")
                                elif compression_ratio < 5:
                                    st.warning("⚠️ Very low compression ratio - memories may be too brief")
                                else:
                                    st.success("✅ Good compression ratio for memory extraction")
                            elif empty_results:
                                st.info("ℹ️ LLM correctly identified no memorable content")
                            else:
                                st.error("❌ Processing succeeded but no memories or errors recorded")
