...
class ActionItemsService(TranscriptService):
    """
    MongoDB-based action items service with full CRUD operations.
    Implements the TranscriptService interface for extensible processing.
    """
    ...
    def get_router(self) -> TypingOptional[APIRouter]:
        """Return the FastAPI router for action items endpoints."""
        from .action_items_api import create_action_items_router
        return create_action_items_router(self)
    ...
    async def process_transcript(self, transcript_text: str, client_id: str, 
        ...
        - Returns processing results in the standard format
        """
            ...
            if keyphrase_pattern.search(transcript_text):
                ...
                try:
                    ...
                    if action_item_count > 0:
                        ...
                        return {
                            "success": True,
                            "count": action_item_count,
                            "message": f"Extracted {action_item_count} action items from 'Simon says' keyphrase",
                            "data": {"keyphrase_detected": True}
                        }
                    else:
                        ...
                        return {
                            "success": True,
                            "count": 0,
                            "message": "No action items found in 'Simon says' transcript segment",
                            "data": {"keyphrase_detected": True}
                        }
                    ...
                    return {
                        "success": False,
                        "count": 0,
                        "message": f"Error processing 'Simon says' action items: {str(e)}",
                        "data": {"keyphrase_detected": True}
                    }
            else:
                ...
                return {
                    "success": True,
                    "count": 0,
                    "message": "No 'Simon says' keyphrase found",
                    "data": {"keyphrase_detected": False}
                }
    ...
    async def extract_and_store_action_items(self, transcript: str, client_id: str, audio_uuid: str, user_id: str, user_email: str) -> int:
        ...
        try:
            ...
            async def _extract_and_store():
                ...
                action_items = await self._extract_action_items_from_transcript(transcript, audio_uuid)
    ...
    async def _extract_action_items_from_transcript(self, transcript: str, audio_uuid: str) -> List[Dict[str, Any]]:
