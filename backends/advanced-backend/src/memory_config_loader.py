...
class MemoryConfigLoader:
    ...
    def __init__(self, config_path: str | None = None):
        ...
        if config_path is None:
            ...
            config_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "memory_config.yaml")
    ...
    def get_memory_prompt(self) -> str:
        ...
        return self.get_memory_extraction_config().get("prompt", "Extract anything relevant about this conversation.")
    
    def get_fact_prompt(self) -> str:
        """Get the fact extraction prompt."""
        return self.get_fact_extraction_config().get("prompt", "Extract specific facts from this conversation.")
    
    def get_action_item_prompt(self) -> str:
        """Get the action item extraction prompt."""
        return self.get_action_item_extraction_config().get("prompt", "Extract action items from this conversation.")
    ...
    def should_skip_conversation(self, conversation_text: str) -> bool:
        ...
        if len(conversation_text) < min_length:
            config_logger.debug(f"Skipping conversation: too short ({len(conversation_text)} < {min_length})")
            return True
        
        if len(conversation_text) > max_length:
            config_logger.debug(f"Skipping conversation: too long ({len(conversation_text)} > {max_length})")
            return True
        ...
        if quality_config.get("skip_low_content", False):
            ...
            if len(words) > 0:
                filler_words = {'um', 'uh', 'hmm', 'yeah', 'ok', 'okay', 'like', 'you', 'know', 'so', 'well'}
                meaningful_words = [word for word in words if word.lower() not in filler_words and len(word) > 2]
                content_ratio = len(meaningful_words) / len(words)
                
                if content_ratio < min_content_ratio:
                    config_logger.debug(f"Skipping conversation: low content ratio ({content_ratio:.2f} < {min_content_ratio})")
                    return True
    ...
    def get_processing_timeout(self) -> int:
        ...
        return self.get_processing_config().get("processing_timeout", 300)
