...
from .persistent_queue import PersistentQueue, PersistentMessage, get_persistent_queue
...
class RecoveryManager:
    ...
    async def start(self, recovery_interval: int = 30):
        ...
        self.recovery_task = asyncio.create_task(
            self._recovery_loop(recovery_interval)
        )
    ...
    async def _retry_item(self, item: QueueItem, rule: RecoveryRule):
        ...
        success = self.queue_tracker.update_item_status(
            item.id, QueueStatus.RETRY, 
            f"Recovered from stale processing state"
        )
    ...
    async def _escalate_item(self, item: QueueItem, rule: RecoveryRule):
        ...
        success = self.queue_tracker.update_item_status(
            item.id, QueueStatus.DEAD_LETTER, 
            "Escalated due to persistent failures"
        )
        ...
        for queue_type in QueueType:
            try:
                ...
                for item in stale_items:
                    ...
                    self.queue_tracker.update_item_status(
                        item.id, QueueStatus.PENDING,
                        "Reset from processing state on startup"
                    )
    ...
    async def manual_recovery(self, queue_type: QueueType, 
...
def init_recovery_manager(queue_tracker: Optional[QueueTracker] = None,
                         persistent_queue: Optional[PersistentQueue] = None):
