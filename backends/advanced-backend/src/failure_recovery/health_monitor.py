    ...
    async def start(self):
        ...
        for name, check in self.health_checks.items():
            task = asyncio.create_task(
                self._monitor_service(name, check)
            )
    ...
    async def _run_health_check(self, name: str, health_check: HealthCheck):
        ...
        try:
            ...
            if result:
                ...
                if (service_health.consecutive_failures >= health_check.failure_threshold and
                    health_check.recovery_callback):
            ...
            if deepgram_key:
                ...
                try:
                    reader, writer = await asyncio.wait_for(
                        asyncio.open_connection(asr_host, asr_port),
                        timeout=3.0
                    )
                    writer.close()
...
def init_health_monitor(recovery_manager: Optional[RecoveryManager] = None,
                       queue_tracker: Optional[QueueTracker] = None,
                       persistent_queue: Optional[PersistentQueue] = None):
