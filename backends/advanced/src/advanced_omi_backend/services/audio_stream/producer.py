...
class AudioStreamProducer:
    ...
    async def init_session_with_speech_detection(
        self,
        client_state,
        user_id: str,
        user_email: str,
        ...
        """
        Initialize streaming session and enqueue speech detection job (V2 architecture).

        This is a convenience method that combines:
        1. Session UUID generation
        2. Session initialization in Redis
        3. Speech detection job enqueueing

        Args:
            client_state: Client state object to store session info
            user_id: User identifier
            user_email: User email
            client_id: Client identifier
            provider: Transcription provider (default: deepgram)
        ...
        logger.info(f"🆔 Created stream session: {session_id} for {client_id}")
        ...
        # Enqueue speech detection job with callbacks
        from advanced_omi_backend.workers.transcription_jobs import (
            stream_speech_detection_job,
            on_speech_detection_success,
            on_speech_detection_failure
        )
        ...
        speech_job = transcription_queue.enqueue(
            ...
            on_success=on_speech_detection_success,
            on_failure=on_speech_detection_failure,
            meta={
                'session_id': session_id,
                'user_id': user_id,
        ...
        logger.info(f"✅ Enqueued speech detection job {speech_job.id} for session {session_id[:12]} (with callbacks)")
        ...
        logger.info(f"✅ Enqueued audio persistence job {audio_job.id} for session {session_id[:12]}")

        return session_id

    async def finalize_session_complete(self, client_state, client_id: str):
        """
        Complete session finalization and cleanup (V2 architecture).

        This is a convenience method that combines:
        1. Sending end-of-session signal to workers
        2. Finalizing session in Redis
        3. Cleaning up client state attributes

        Args:
            client_state: Client state with session info
            client_id: Client identifier
        """
        if not hasattr(client_state, 'stream_session_id'):
            logger.debug(f"No active session to finalize for {client_id}")
            return

        session_id = client_state.stream_session_id

        # Send end-of-session signal to trigger final transcription
        await self.send_session_end_signal(session_id)
        logger.info(f"📡 Sent end-of-session signal for {session_id}")

        # Finalize session (cleanup)
        await self.finalize_session(session_id)
        logger.info(f"✅ Finalized stream session {session_id}")

        # Clear session state attributes
        if hasattr(client_state, 'stream_session_id'):
            delattr(client_state, 'stream_session_id')
        if hasattr(client_state, 'stream_chunk_count'):
            delattr(client_state, 'stream_chunk_count')
        if hasattr(client_state, 'stream_audio_chunks'):
            delattr(client_state, 'stream_audio_chunks')
        if hasattr(client_state, 'stream_audio_format'):
            delattr(client_state, 'stream_audio_format')
        if hasattr(client_state, 'speech_detection_job_id'):
            delattr(client_state, 'speech_detection_job_id')

        logger.debug(f"🧹 Cleared session state for {client_id}")
