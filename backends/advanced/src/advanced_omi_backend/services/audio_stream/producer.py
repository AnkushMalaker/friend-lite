...
class AudioStreamProducer:
    ...
    async def init_session(
        ...
        """
        # Provider-specific stream naming (allows consumer groups to scale horizontally)
    ...
    async def add_audio_chunk(
        ...
        if session_id not in self.session_buffers:
            stream_name = f"audio:stream:deepgram"  # Default to deepgram if init_session not called
            self.session_buffers[session_id] = {
                "buffer": b"",
                "chunk_count": 0,
                "user_id": user_id,
