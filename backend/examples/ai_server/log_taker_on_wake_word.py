...
from utils import llm_refactor, record_till_silence, save_segment
...
class Porcupine:
    def __init__(self):
        self.porcupine = pvporcupine.create(
            ...
            keyword_paths=[
                "models/keyword/Butler_en_linux_v3_0_0/Butler_en_linux_v3_0_0.ppn"
            ],
        )

    def process(self, audio_frame: NumpyFrame) -> int:
...
async def main_async():
    ...
    while True:
        async for audio_chunk in audio_stream:
            keyword = porcupine.process(audio_chunk)
            if keyword > -1:
                logger.info("Keyword detected, starting to record")
                ...
                await save_segment(
                    recorded_audio, RECORDING_DIR / f"recording_{timestamp}.wav"
                )
